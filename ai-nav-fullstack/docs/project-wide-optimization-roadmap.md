...
当前阶段说明（2026-07-15）：非 Agent 模块已经完成落地与归档。用户随后明确按 `docs/agent-development-handoff.md` 和 `docs/agent-delivery-roadmap.md` 启动 Agent 专项，后续状态不再由本早期总路线维护。
...
稳定公开接口；模块内部优先直接调用。详细标准见 `docs/modular-monolith-guidelines.md`。
...
当前执行任务和阶段门禁已迁移到 `docs/agent-delivery-roadmap.md`；以下清单保留为项目级能力摘要。
...
- [ ] 再接入模型 provider port、流式响应、会话和评估。
- [ ] 建立 QA、导航、推荐、学习计划、安全和越权评估集。

验收：替换模型或 Agent 框架不影响 Learning、Tools、Users 的实现与数据。

### 5.6 Frontend（P1）

- [ ] 明确 5 个现有页面和未来 Agent 页的唯一入口脚本。（非 Agent 五页已完成；Agent 页按决定冻结）
...
- [ ] 增加内容运营审计，区分业务数据与用户私有数据。
- [ ] 为 SQLite 设置 WAL、busy timeout 和备份策略；达到并发阈值后再迁 PostgreSQL。
...
- [ ] API 契约与权限测试。
- [ ] Agent 工具与引用真实性测试。
- [ ] Playwright 关键流程与视觉回归。
- [ ] 记录 operation、latencyMs、resultCount、errorCode、requestId。
- [ ] 建立查询基线、慢查询检查和前端性能预算。
