...
- 已新增 `scripts/freeze-users-contracts.py`，生成 `docs/06-evidence/users/auth_users_openapi.json` 和 `docs/06-evidence/users/auth_users_response_shapes.json`，冻结 auth/users OpenAPI 与真实服务响应字段形状。
- 已新增 `scripts/check-users-security-baseline.py`，生成 `docs/06-evidence/users/users_security_baseline.json`；当前结果为 3 pass、3 known-risk、0 fail。已知风险为默认开发密钥、宽松 CORS、refresh token 存在 localStorage。
- 已新增 `scripts/benchmark-users.py`，生成 `docs/06-evidence/users/users_performance_baseline.json`，记录当前用户关键链路耗时和 SQL 语句数：current_user p95 约 0.9ms/1 statement，refresh p95 约 1.7ms/2 statements，profile/preferences/sessions 读取 p95 均小于 1ms/1 statement。
...
- 已冻结设置页认证态截图到 `docs/06-evidence/users/screenshots/`：`settings-desktop-1440.png`、`settings-mobile-390.png`，并保存对应 JSON 与 `settings-screenshots-manifest.json`。
...
- 已刷新 `docs/06-evidence/users/auth_users_response_shapes.json`、`tests/contracts/users_contract_v1.json` 和 `docs/06-evidence/users/users_performance_baseline.json`；sessions list 当前基线为 2 statements（count + page list）。
...
- 审计规范见 `docs/03-domains/users/users-audit-event-catalog.md`；IP 与 User-Agent 只从服务端请求上下文采集并限制长度。
...
- 生命周期与边界见 `docs/03-domains/users/users-privacy-lifecycle.md`，新增隐私审计事件已同步到 `docs/03-domains/users/users-audit-event-catalog.md`。
...
- 资产边界和 API 见 `docs/03-domains/users/users-assets-contract.md`；新增四类资产审计事件并同步事件目录。`USR-501` 至 `USR-505` 全部完成。
...
- 可观测性契约见 `docs/03-domains/users/users-observability.md`；明确多 worker/多实例需接入集中式指标系统，且不得增加用户身份 label。
...
- OpenAPI 冻结扩展到隐私、用户学习状态、资产、Agent 保存和管理员指标，共 46 条路径。最终验收脚本汇总 OpenAPI、数据库、前端、四视口、安全、性能与恢复演练，报告见 `docs/03-domains/users/users-final-acceptance-report.md`。
...
- 安全基线新增 `USR-SEC-027`；路线图 Users/Auth 5.3 最后一项已按风险等价保障收口。详细见 `docs/03-domains/users/users-command-safety-matrix.md`。
