...
> 当前实现更新（2026-07-25）：密码恢复已经改为已验证外部通道的一次性凭据。
> 服务端只保存 HMAC，凭据绑定用户、用途、有效期和单次消费状态；完成后撤销全部
> 会话。密保问题仍可用于账号安全设置，但不再是独立恢复因子。旧
> `/password-reset/security/*` 路径仅作为弃用兼容入口，其中 verify 固定返回 410。
> 当前未接入真实邮件或短信服务，默认发送端 fail-closed，此项属于外部上线阻塞。
>
> Web 认证更新（2026-07-25）：Access Token 默认 10 分钟且只驻留页面模块内存；
> 页面刷新通过 HttpOnly Refresh Cookie 恢复。服务端登录、注册与刷新响应不返回
> Refresh Token，前端加载时会清理旧 localStorage token key。Auth 输出使用所属领域
> 的字段级 schema，现有 JSON API 均有输出边界验证。
