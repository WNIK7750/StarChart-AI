...
详细文件和验证结果见 `docs/03-domains/learning/learning-area-implementation.md`。学习核心后续只接受契约内迭代；内容运营界面、Agent 计划编排和平台级基础设施由各自分区继续实现。
...
状态说明（2026-07-25）：Phase 0 至 Phase 4 已完成；Phase 2 的专项样例要求已由当前 30 条版本化 Agent 评估集和独立 Learning/Agent 测试覆盖。Phase 5 的个性化计划持久化以及 Phase 6 的平台级扩展项已按第 1.1 节边界移交为证据触发的独立范围，不是当前版本的未完成任务。

### Phase 0：契约冻结与回归基线（P0）

- [x] 保存现有三个学习接口的真实响应样例。
- [x] 为路线图、资源和节点详情建立 Pydantic 响应模型。
- [x] 增加 API 契约测试和节点 404 测试。
- [x] 列出 `learn.html`、`learn-node.html` 实际加载脚本，标记旧双轨代码。
- [x] 建立桌面与移动端截图基线。
...
- [x] 创建 `backend/app/learning` 包及 DTO、ports、service。
- [x] 将学习 SQL 迁入 SQLite repository。
- [x] 将时长、主资料、导航与领域筛选规则迁入 policy/service。
- [x] 将 learning router 改为薄适配器，并保持旧接口响应兼容。
- [x] 使用临时 SQLite fixture 对 service 和 repository 分层测试。

验收：learning router 不再包含 SQL；service 测试不需要启动 FastAPI；现有页面无回归。

### Phase 2：搜索与 Agent 只读服务（P0）

- [x] 建立学习内容搜索契约和标准化关键词策略。
- [x] 实现 `search_learning` 与 `get_agent_context`。
- [x] 建立统一站内学习链接生成器和引用结构。
- [x] 扩展 Agent learning tools，仅调用学习 service。
- [x] 在 Agent evaluator 中校验节点 slug、引用和站内链接。
- [x] 由 30 条版本化 Agent 评估集、Learning 契约测试和 Agent 服务测试共同覆盖学习检索/导航。

验收：Agent 能用站内真实数据返回节点、理由、先修和有效链接；无结果时不编造内容。

### Phase 3：前端模块化与体验地基（P1）

- [x] 从 `v2-api.js` 抽出学习 API、store 和 view modules。
- [x] 去除页面内联 `domainNodes` 业务副本和重复事件绑定。
- [x] 清理或归档未使用的 `learn.js/roadmap.js/node.js` 双轨实现。
- [x] 实现统一 loading、empty、error、retry 状态。
- [x] 增加节点先修、下一步和可访问性语义。
- [x] 完成 360px、768px、1440px 视口视觉与交互回归。

验收：学习页面入口只负责编排；没有从 DOM 反推业务数据；键盘可访问且移动端无溢出。

### Phase 4：用户进度与继续学习（P1）

- [x] 在用户层增加进度、活动和收藏迁移脚本。
- [x] 创建用户层 `learning_state` repository、service 和薄路由。
- [x] 由用户层落地进度写入、最近阅读、继续学习和收藏接口。
- [x] 用户层聚合学习服务的公开事实，禁止复制学习内容快照作为主数据。
- [x] 首页、学习页、节点页和设置页消费用户层真实状态，不制造默认进度。
- [x] 为幂等、越权、并发覆盖和未登录写入增加测试。

验收：用户可跨会话继续学习并查看最近阅读；不同用户数据隔离；重复提交不会产生错误进度；学习服务在无用户模块时仍可独立运行。

### Phase 5：学习计划服务与 Agent 增强（P2）

- 独立范围：只有个性化计划需求经评估成立后，再基于语义边、用户水平和时间约束实现规则型计划预览。
- 独立范围：届时计划结果包含节点、顺序、理由、预计时长和引用。
- 独立范围：Agent 只能在规则结果之上生成自然语言说明，不得修改事实字段。
- 独立范围：保存必须由用户确认并通过 Users Command，且不依赖 Agent 会话。
- 独立范围：立项时同步建立计划完整度、先修合法性和时长约束评估。

验收：关闭 LLM 时仍能生成合法的基础计划；更换 Agent 框架不影响学习服务与数据。

### Phase 6：运营、观测与内容治理（P2）

- [x] 增加学习服务日志：operation、latencyMs、errorCode、viewerType，并由调用方记录结果数量。
- [x] 增加资源链接检查、内容版本和下线流程。
- [x] 建立服务性能基线与慢查询检查。
- 独立范围：出现公开读取滥用或跨实例缓存需求后，再增加 Learning 专属限流/分布式缓存；当前已具备短时公开缓存策略。
- 独立范围：只有新增 Agent 学习写操作或个性化计划能力时，才建立对应 feature flag；当前不存在需关闭的该类写能力。
