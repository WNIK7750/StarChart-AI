...
> 状态：独立复验、本地整改及 deterministic HTTP 测试部署完成；生产发布继续 NO-GO
> 复验日期：2026-07-25；服务器部署补充：2026-07-28
...
- **deterministic HTTP 测试部署：GO。** 2026-07-28 的不可变发布、Nginx/systemd、
  8001 loopback、隔离单账号数据库、旧站兼容路由、确定性 smoke 和 24 项交互式账号/
  游客全功能流均通过；脱敏证据见
  `docs/06-evidence/platform/http-test-server-validation.json`。该结论仅适用于公开 HTTP
  测试环境，不代表真实 Provider 或生产发布。
...
2026-07-28 补充：deterministic HTTP 测试服务器已经验证，不再属于“服务器完全未执行”；
真实 Provider 预览、HTTPS、容量、合规、备份恢复、回滚演练和外部生产签收仍为
`NOT RUN`。备份归档存在且压缩完整性通过，不能替代恢复验证。
