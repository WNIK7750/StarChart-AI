...
普通 8088 本地档现在通过显式 `AI_NAV_AGENT_GUEST_CHAT_ENABLED=1` 开放
deterministic 游客问答；`start.ps1` 仅在调用方未配置该变量时设置本地默认值。
`/api/v1/runtime/public` 与实际游客接口均返回可用状态，前端发送前也会核对该公开
能力。该本地开关只在 `development` 与 `test` 档生效，不能替代隔离
`http_test` 部署，更不能替代真实服务器 Nginx、systemd 或发布后公网验证。

本轮已逐项验证四个固定问题：“RAG 怎么学？”、“推荐免费优先的代码工具”、
“带我去学习 Transformer”和“帮我做一个论文阅读工作流”。四项均返回 200 和
站内证据；若最终证据为空，统一返回固定的“暂时没有找到足够相关的站内内容”
句式。桌面与移动端均保持 `/assistant`，没有 `Not Found`、红色错误行、相关
控制台错误或横向溢出。
