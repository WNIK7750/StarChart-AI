...
**Goal:** Make `<PROJECT_ROOT>` the only Git repository root and make its project files appear directly at the root of `WNIK7750/StarChart-AI`.
...
**Execution status (2026-07-29):** The project-root working tree and root-relative
workflow are present, but the first `master` migration commit retained the old
parent repository as its Git parent. A clean `git subtree split` candidate at
`8470709aa44463fbcec0e46d784d4e7e2e266d7e` is therefore being independently
verified before any further default-branch change. The existing `master` remains
recoverable and unchanged during this verification.
...
- The final local Git root must be `<PROJECT_ROOT>`.
...
**Interfaces:**
- Consumes: parent Git repository at `<PARENT_REPOSITORY_ROOT>`
- Produces: immutable bundle backup and a recorded source commit ID

- [x] **Step 1: Confirm repository boundaries and current state**
...
Expected: the old root is `<PARENT_REPOSITORY_ROOT>`, and the current workspace changes remain visible.

- [x] **Step 2: Confirm backup storage is ignored**
...
- [x] **Step 3: Create and verify a complete bundle**
...
- [x] **Step 1: Clone the bundle into an isolated directory**
...
- [x] **Step 2: Extract the project prefix**
...
- [x] **Step 1: Copy the existing workflow into the candidate**
...
- [x] **Step 2: Remove the obsolete project prefix**
...
- [x] **Step 3: Validate workflow paths**
...
- Create locally: `.git` inside `<PROJECT_ROOT>`
...
After preserving the old parent repository and current workspace, make `<PROJECT_ROOT>` an independent clone of the corrected remote and verify:

```powershell
git rev-parse --show-toplevel
```

Expected: output is exactly `<PROJECT_ROOT>`.
